...
# ========== 自定义DashScope向量Embedding类（原生SDK，不走兼容网关） ==========
class DashScopeEmbeddings(Embeddings):
    def __init__(self, model_name: str, api_key: str):
        self.model_name = model_name
        self.api_key = api_key
        self.batch_max = 10  # v4单批最大10条

    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        """批量文档向量化，自动分片，使用status_code判断成功与否"""
        # 过滤空文本
        texts = [t for t in texts if t is not None and t.strip() != ""]
        if len(texts) == 0:
            return []

        all_embeddings = []
        for i in range(0, len(texts), self.batch_max):
            batch_texts = texts[i:i + self.batch_max]
            print(f"正在处理批次 {i//self.batch_max +1}, 文本数量:{len(batch_texts)}")
            try:
                ...
                raise RuntimeError(f"调用DashScope网络异常：{str(e)}，批次文本：{batch_texts}")
            
            # ✅ 重点修复：用 status_code 判断，不是 resp.code
            if resp.status_code != 200:
                raise RuntimeError(
                    f"DashScope Embedding业务失败！status_code:{resp.status_code}, code:{resp.code}, message:{resp.message}, 批次文本：{batch_texts}"
                )
            # 请求成功，读取向量
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        try:
            ...
            raise RuntimeError(f"embed_query调用DashScope异常：{str(e)}，文本：{text}")
        
        if resp.status_code != 200:
            raise RuntimeError(
                f"DashScope embed_query失败！status_code:{resp.status_code}, code:{resp.code}, message:{resp.message}, 文本：{text}"
            )
...
#     """构建 Langchain OpenAI Embeddings实例 （使用项目统一配置）"""
...
def build_index(force_rebuild:bool=False)->Chroma:
    ...
    if existing >0 and not force_rebuild:
        print(f"已有索引数据（{existing}条),跳过重建。如需重建请传入 force_rebuild=Ture")
        return vectorstore
    # 清空旧数据（如果 force_rebuild)
    if force_rebuild and existing>0:
        # ChromaDB 较新版本不支持 Where 过滤，用delete逐个删除
    ...
    print(f"索引构建完成：{len(documents)}张表已写入ChromaDB")
