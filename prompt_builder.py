...
示例4：
问题：数据库有几张表，表结构是什么
SQL：SELECT TABLE_NAME, COLUMN_NAME, DATA_TYPE, IS_NULLABLE, COLUMN_KEY, ORDINAL_POSITION FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() ORDER BY TABLE_NAME, ORDINAL_POSITION;
"""
...
- UNION 排序：UNION / UNION ALL 之后的 ORDER BY 只能使用 SELECT 列表中的列或别名；需要按 ORDINAL_POSITION 排序时，必须把它写进 SELECT，禁止只在 ORDER BY 中引用
- 元数据查询：问“有几张表”“表结构”时，直接查 INFORMATION_SCHEMA.COLUMNS / TABLES，不要按表名逐张 UNION
"""
